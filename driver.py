...
def main():
    ...
    parser.add_argument( "-e", "--end", type=int, help="Specify the maximum page that should be considered for each letter (ex: 1) -- defaults to as many as are available." )
    parser.add_argument( "-m", "--members", action="store_true", help="Flag to request to return only members items -- defaults to FALSE." )
    parser.add_argument( "-s", "--start", type=int, help="Specify the minimum page that should be considered for each letter  (ex: 3) -- defaults to 1." )   
    ...
    parser.add_argument( "-f", "--fact", action="store_false", help="Flag to request pricing and quantity information is fetched." )
    parser.add_argument( "-r", "--materials", action="store_true", help="Flag to request required materials are resolved." )
